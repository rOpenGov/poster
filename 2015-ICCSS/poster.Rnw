...
\begin{minipage}{0.69\textwidth}
  ...
  p <- p + coord_map(project="orthographic", xlim=c(-10,34), ylim=c(35,65))
  p <- p + labs(title = paste0("Disposable household incomes in ",max(df$time)))
  p <- p +  theme(legend.position = c(0.23,0.50), 
...
#                           panel.grid.major = element_line(colour = 'Grey80', size = .5, linetype = 'solid'),
  ...
  <<page4-2, eval=TRUE, dev='pdf', fig.height=21, fig.width=21, cache=FALSE, message=FALSE, warning=FALSE, echo=FALSE>>=
  print(p)
